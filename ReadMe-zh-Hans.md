...
VC-LTL创建初衷是为了解决`运行库部署问题`以及 `Fls的127+1上限问题`。在大型项目中往往有众多模块，如果都采用静态编译那么造成的空间浪费先不说，最后程序也会因为 Fls上限导致程序无法正常运行。

而VC-LTL能让你的项目如同系统文件一样共享系统内置`msvcrt.dll`，有效的解决Fls上限以及运行时部署问题，同时大大缩减程序体积。
