...
;
        ...
        prfum   pldl1strm, [x1]                         ; preload the first cache line
        cmp     x2, #8                                  ; less than 8 bytes?
...
;
        ...
        subs    x2, x2, #32                             ; pre-decrement to simplify the loop
        blo     %F12                                    ; skip over the loop if we don't have enough
        prfum   pldl1strm, [x1, #-64]                   ; prefetch the next cacheline backwards
        subs    x2, x2, #64                             ; pre-decrement to simplify the loop
        blo     %F11                                    ; skip over the loop if we don't have enough
10
        sub     x1, x1, #128
        ...
        prfum   pldl1strm, [x1, #-128]                  ; prefetch a cacheline one block backwards
        prfum   pldl1strm, [x1, #-192]                  ; prefetch another cacheline backwards
