...
public:
	...
	explicit DName(pcchar_t, StringLife life);
	explicit DName(pcchar_t&, char, StringLife life);
...
private:
	...
	void append(const DNameNode *newRight);
	void doPchar(char);
	void doPchar(pcchar_t, int, StringLife life);
};
    ...
    )
{
	...
	if (result.status() == DN_error)
		...
		unDName = DName(name, StringLife::Persistent);	// Return the original name
	else
		unDName = result;
...
#define GENERIC_TYPE_LEN 13
			...
			if (isGenericType)
			{
				...
				if (haveTemplateParameters() && (m_pGetParameter != nullptr))
				{
					...
					{
						zName = DName(str, StringLife::Unknown);
					}
					else
					{
			...
			else
			{
				...
				zName = DName(gName, '@', StringLife::Persistent);	// This constructor updates 'name'
			}
		}
	...
	{	//	Insert the scope operator if not the first scope
		...
		if (*gName == '?')
			...
			case '%':
			{
				...
				//
				DName namespaceName = DName(gName, '@', StringLife::Persistent);
...
DName UnDecorator::getDimension(bool fSigned)
{
	...
	StringLiteral prefix = "`non-type-template-parameter"_l;
	...
	if (!*gName)
		...
		return hasPrefix ? (prefix + DName((unsigned __int64)(*gName++ - '0' + 1))) : DName((unsigned __int64)(*gName++ - '0' + 1));
	else
	{
		...
		if (*gName++ != '@')
			return DName(DN_invalid);		// Should never get here

		if (fSigned)
		{
			return hasPrefix ? (prefix + DName((__int64)dim)) : DName((__int64)dim);
		}
		else
		{
			return hasPrefix ? (prefix + DName(dim)) : DName(dim);
		}
...
DName UnDecorator::getTemplateNonTypeArgument(void)
{
	...
	case TC_fp:
	{
		...
		if (mantissa.isValid() && exponent.isValid())
		{
			...
			return DName(buf, StringLife::Temporary) + 'e' + exponent;
		}
		else
		{
			return DName(DN_truncated);
	...
	case TC_generic_method_parameter:
	{
		...
		if (haveTemplateParameters() && (m_pGetParameter != nullptr))
		{
			...
			{
				return DName(pName, StringLife::Unknown);
			}
		}

		sprintf_s(buffer, _countof(buffer), "%d", tpi.number_);
		DName index(buffer, StringLife::Temporary);
...
DName UnDecorator::getTemplateTypeArgument(void)
{
	...
	else if (*gName == '?')
	{
		...
		if (haveTemplateParameters() && (m_pGetParameter != nullptr))
		{
			...
			if (const char* str = (*m_pGetParameter)(atol(buffer)))
			{
				return DName(str, StringLife::Unknown);
			}
			else
			{
