...
FTYPE* FNAME(Xp_mulx)(FTYPE* p, int n, const FTYPE* q, int m,
    FTYPE* ptemp2) { // multiply by an extended precision value (needs 2*n temp)
    if (n != 0 && m != 0) {
...
FTYPE* FNAME(Xp_sqrtx)(
    FTYPE* p, int n, FTYPE* ptemp4) { // find square root of an extended precision value (needs 4*n temp)
    if (n != 0) {
