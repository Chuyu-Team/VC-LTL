...
long long _Xtime_get_ticks() { // get system time in 100-nanosecond intervals since the epoch
    ...
    return (((long long) ft.dwHighDateTime) << 32) + (long long) ft.dwLowDateTime - EPOCH;
}
...
long _Xtime_diff_to_millis2(const xtime* xt1, const xtime* xt2) { // convert time to milliseconds
    ...
    return (long) (diff.sec * MSEC_PER_SEC + (diff.nsec + NSEC_PER_MSEC - 1) / NSEC_PER_MSEC);
}
...
_CRTIMP2_PURE long long __cdecl _Query_perf_counter(void) { // get current value of performance counter
    LARGE_INTEGER li;
    QueryPerformanceCounter(&li); // always succeeds
    return li.QuadPart;
}

_CRTIMP2_PURE long long __cdecl _Query_perf_frequency(void) { // get frequency of performance counter
    LARGE_INTEGER li;
