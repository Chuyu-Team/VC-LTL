...
#define READONLY_PERMS perms(perms::all & ~WRITE_PERMS)
#define WRITE_PERMS perms(perms::owner_write | perms::group_write | perms::others_write)
...
// DIRECTORY FUNCTIONS
static TCHAR* _Strcpy(TCHAR (&_Dest)[_MAX_FILESYS_NAME], const TCHAR* _Src) { // copy an NTBS
    TSTRCPY(_Dest, _MAX_FILESYS_NAME, _Src);
    return _Dest;
}

static HANDLE _FilesysOpenFile(const TCHAR* _Fname, DWORD _Desired_access, DWORD _Flags) {
#if defined(_CRT_APP)
...
_FS_DLL TCHAR* __CLRCALL_PURE_OR_CDECL _Read_dir(
    TCHAR (&_Dest)[_MAX_FILESYS_NAME], void* _Handle, file_type& _Ftype) { // read a directory entry
    TFUN(WIN32_FIND_DATA) _Dentry;

    for (;;) {
        if (TFUN(FindNextFile)((HANDLE) _Handle, &_Dentry) == 0) { // fail
        ...
        if (_Dentry.cFileName[0] != TLIT('.')
            || (_Dentry.cFileName[1] != TLIT('\0')
                   && (_Dentry.cFileName[1] != TLIT('.')
                          || _Dentry.cFileName[2] != TLIT('\0')))) { // not "." or "..", get file type and return name
...
_FS_DLL int __CLRCALL_PURE_OR_CDECL _To_wide(const char* _Bsrc, wchar_t* _Wdest) { // return nonzero on success
    return MultiByteToWideChar(_Filesys_code_page(), 0, _Bsrc, -1, _Wdest, _MAX_FILESYS_NAME);
}

_FS_DLL int __CLRCALL_PURE_OR_CDECL _To_byte(const wchar_t* _Wsrc, char* _Bdest) { // return nonzero on success
    return WideCharToMultiByte(_Filesys_code_page(), 0, _Wsrc, -1, _Bdest, _MAX_FILESYS_NAME, NULL, NULL);
}


_FS_DLL void* __CLRCALL_PURE_OR_CDECL _Open_dir(TCHAR (&_Dest)[_MAX_FILESYS_NAME], const TCHAR* _Dirname, int& _Errno,
    file_type& _Ftype) { // open a directory for reading
    TFUN(WIN32_FIND_DATA) _Dentry;
    ...
    void* _Handle =
        TFUN(FindFirstFileEx)(_Wildname.c_str(), FindExInfoStandard, &_Dentry, FindExSearchNameMatch, NULL, 0);
    if (_Handle == INVALID_HANDLE_VALUE) { // report failure
        _Errno = ERROR_BAD_PATHNAME;
        *_Dest = TLIT('\0');
        return 0;
    } else { // success, get first directory entry
        _Errno = 0;
        if (_Dentry.cFileName[0] == TLIT('.')
            && (_Dentry.cFileName[1] == TLIT('\0')
                   || _Dentry.cFileName[1] == TLIT('.') && _Dentry.cFileName[2] == TLIT('\0'))) { // skip "." and ".."
            _Read_dir(_Dest, _Handle, _Ftype);
            if (_Dest[0] != TLIT('\0')) {
                return _Handle;
            } else { // no entries, release handle
                _Close_dir(_Handle);
                return 0;
            }
        } else { // get file type and return handle
...
_FS_DLL bool __CLRCALL_PURE_OR_CDECL _Current_get(TCHAR (&_Dest)[_MAX_FILESYS_NAME]) { // get current working directory
    _Strcpy(_Dest, TLIT(""));
#if defined(_CRT_APP)
...
_FS_DLL bool __CLRCALL_PURE_OR_CDECL _Current_set(const TCHAR* _Dirname) { // set current working directory
#if defined(_CRT_APP)
    (void) (_Dirname);
    return false; // no support
#else // defined(_CRT_APP)
...
_FS_DLL TCHAR* __CLRCALL_PURE_OR_CDECL _Symlink_get(
    TCHAR (&_Dest)[_MAX_FILESYS_NAME], const TCHAR*) { // get symlink -- DUMMY
    _Dest[0] = TCHAR(0);
    return &_Dest[0];
}

_FS_DLL TCHAR* __CLRCALL_PURE_OR_CDECL _Temp_get(TCHAR (&_Dest)[_MAX_FILESYS_NAME]) { // get temp directory
    TCHAR _Dentry[MAX_PATH];
    return _Strcpy(_Dest, TFUN(GetTempPath)(MAX_PATH, &_Dentry[0]) == 0 ? TLIT(".") : &_Dentry[0]);
}


_FS_DLL int __CLRCALL_PURE_OR_CDECL _Make_dir(const TCHAR* _Fname,
    const TCHAR*) { // make a new directory (ignore attributes)
...
_FS_DLL bool __CLRCALL_PURE_OR_CDECL _Remove_dir(const TCHAR* _Fname) { // remove a directory
    return TRMDIR(_Fname) != -1;
}

// FILE STATUS FUNCTIONS
_FS_DLL file_type __CLRCALL_PURE_OR_CDECL _Stat(const TCHAR* _Fname, perms* _Pmode) { // get file status
    WIN32_FILE_ATTRIBUTE_DATA _Data;

    if (TFUN(GetFileAttributesEx)(_Fname, GetFileExInfoStandard, &_Data)) { // get file type and return permissions
        if (_Pmode != 0) {
            *_Pmode = _Data.dwFileAttributes & FILE_ATTRIBUTE_READONLY ? READONLY_PERMS : perms::all;
        }

        return _Map_mode(_Data.dwFileAttributes);
    } else { // invalid, get error code
...
_FS_DLL file_type __CLRCALL_PURE_OR_CDECL _Lstat(const TCHAR* _Fname, perms* _Pmode) { // get symlink file status
    return _Stat(_Fname, _Pmode); // symlink not supported
}

_FS_DLL unsigned long long __CLRCALL_PURE_OR_CDECL _Hard_links(const TCHAR* _Fname) { // get hard link count
...
_FS_DLL unsigned long long __CLRCALL_PURE_OR_CDECL _File_size(const TCHAR* _Fname) { // get file size
    WIN32_FILE_ATTRIBUTE_DATA _Data;
...
_FS_DLL int64_t __CLRCALL_PURE_OR_CDECL _Last_write_time(const TCHAR* _Fname) { // get last write time
    WIN32_FILE_ATTRIBUTE_DATA _Data;

    if (!TFUN(GetFileAttributesEx)(_Fname, GetFileExInfoStandard, &_Data)) {
        return -1;
    } else { // success, convert time
...
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Set_last_write_time(const TCHAR* _Fname,
    int64_t _When) { // set last write time
...
_FS_DLL space_info __CLRCALL_PURE_OR_CDECL _Statvfs(const TCHAR* _Fname) { // get space information for volume
    space_info _Ans  = {static_cast<uintmax_t>(-1), static_cast<uintmax_t>(-1), static_cast<uintmax_t>(-1)};
    TSTRING _Devname = _Fname;

    if (_Devname.empty() || _Devname.back() != TLIT('/') && _Devname.back() != TLIT('\\')) {
        _Devname.push_back(TLIT('/'));
    }

    _ULARGE_INTEGER _Available, _Capacity, _Free;

    if (TFUN(GetDiskFreeSpaceEx)(_Devname.c_str(), &_Available, &_Capacity, &_Free)) { // convert values
...
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Equivalent(
    const TCHAR* _Fname1, const TCHAR* _Fname2) { // test for equivalent file names
#if defined(_CRT_APP)
...
#if _HAS_HARDLINKS
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Link(const TCHAR* _Fname1, const TCHAR* _Fname2) { // link _Fname2 to _Fname1
    return CreateHardLinkW(_Fname2, _Fname1, 0) != 0 ? 0 : GetLastError();
}


#else // _HAS_HARDLINKS
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Link(const TCHAR*, const TCHAR*) { // link _Fname2 to _Fname1
    return errno = EDOM; // hardlinks not supported
}
#endif // _HAS_HARDLINKS

#if _HAS_SYMLINKS
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Symlink(const TCHAR* _Fname1, const TCHAR* _Fname2) { // link _Fname2 to _Fname1
    return __crtCreateSymbolicLinkW(_Fname2, _Fname1, 0) != 0 ? 0 : GetLastError();
}


#else // _HAS_SYMLINKS
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Symlink(const TCHAR*, const TCHAR*) { // link _Fname2 to _Fname1
    return errno = EDOM; // symlinks not supported
}
#endif // _HAS_SYMLINKS

_FS_DLL int __CLRCALL_PURE_OR_CDECL _Rename(const TCHAR* _Fname1, const TCHAR* _Fname2) { // rename _Fname1 as _Fname2
    return TRENAME(_Fname1, _Fname2) == 0 ? 0 : GetLastError();
}

_FS_DLL int __CLRCALL_PURE_OR_CDECL _Resize(const TCHAR* _Fname, uintmax_t _Newsize) { // change file size
    bool _Ok = false;
...
_FS_DLL int __CLRCALL_PURE_OR_CDECL _Unlink(const TCHAR* _Fname) { // unlink _Fname
    return TREMOVE(_Fname) == 0 ? 0 : GetLastError();
}

_FS_DLL int __CLRCALL_PURE_OR_CDECL _Copy_file(const TCHAR* _Fname1, const TCHAR* _Fname2) { // copy _Fname1 to _Fname2
#if defined(_ONECORE) || defined(_KERNELX)
...
#else // defined(_ONECORE) || defined(_KERNELX)
    return TFUN(CopyFile)(_Fname1, _Fname2, 0) != 0 ? 0 : GetLastError();
#endif // defined(_ONECORE) || defined(_KERNELX)
}


_FS_DLL int __CLRCALL_PURE_OR_CDECL _Chmod(const TCHAR* _Fname, perms _Newmode) { // change file mode to _Newmode
    WIN32_FILE_ATTRIBUTE_DATA _Data;

    if (!TFUN(GetFileAttributesEx)(_Fname, GetFileExInfoStandard, &_Data)) {
        return -1;
    } else { // got mode, alter readonly bit
        ...
        return _Mode == _Oldmode ? 0 : TFUN(SetFileAttributes)(_Fname, _Mode) != 0 ? 0 : -1;
    }
}
