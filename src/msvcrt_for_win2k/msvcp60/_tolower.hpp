...
_CRTIMP2_PURE int __CLRCALL_PURE_OR_CDECL _Tolower(int c, const _Ctypevec* ploc) {
    ...
    if (0
        == (size = __crtLCMapStringA(
                locale, LCMAP_LOWERCASE, (const char*) inbuffer, size, (char*) outbuffer, 3, codepage, TRUE))) {
        return c;
    }
...
_LCRT_DEFINE_IAT_SYMBOL(_Tolower);
...
_CRTIMP2_PURE _Ctypevec __CLRCALL_PURE_OR_CDECL _Getctype() {
    ...
    ctype._Table = (const short*) calloc(256, sizeof(*__pctype_func()));
    if (ctype._Table != 0) {
...
_LCRT_DEFINE_IAT_SYMBOL(_Getctype);
