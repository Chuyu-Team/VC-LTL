...
static size_t __cdecl __wcsrtombs_utf8(char* dst, const wchar_t** src, size_t len, mbstate_t* ps);
...
#if 1
static size_t __cdecl internal_wcsrtombs(
...
static size_t __cdecl __wcsrtombs_utf8(char* dst, const wchar_t** src, size_t len, mbstate_t* ps)
{
    ...
    if (dst != nullptr)
    {
        ...
        for (;;)
        {
            ...
            const size_t retval = c16rtomb(temp, *current_src, ps);
    ...
    else
    {
        ...
        for (;;)
        {
            const size_t retval = c16rtomb(buf, *current_src, ps);
            if (retval == __crt_mbstring::INVALID)
            {
                return retval;
            }
