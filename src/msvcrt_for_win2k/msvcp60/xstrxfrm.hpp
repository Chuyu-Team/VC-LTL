...
_CRTIMP2_PURE size_t __CLRCALL_PURE_OR_CDECL _Strxfrm(
    ...
    if (ploc == 0) {
        locale      = ___lc_handle_func()[LC_COLLATE];
    ...
    if ((locale == 0) && (codepage == CP_ACP)) {
        ...
        if (0
            != (dstlen =
                    __crtLCMapStringA(locale, LCMAP_SORTKEY, _string2, (int) _n2, nullptr, 0, codepage, TRUE))) {
            retval = dstlen;

            // if not enough room, return amount needed
            if (dstlen <= (int) (_n1)) {
                // Map src string to dst string
                __crtLCMapStringA(locale, LCMAP_SORTKEY, _string2, (int) _n2, _string1, (int) _n1, codepage, TRUE);
            }
        }
    }
...
_LCRT_DEFINE_IAT_SYMBOL(_Strxfrm);
