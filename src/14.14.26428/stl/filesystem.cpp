...
template<class _Fn_ptr>
	[[nodiscard]] inline _Fn_ptr __stdcall _Runtime_dynamic_link(volatile _STD _Identity_t<_Fn_ptr> * const _Cache,
	...
	if (_Result)
		{
		return (__crt_fast_decode_pointer(_Result));
		}
	...
	__crt_interlocked_exchange_pointer(_Cache, __crt_fast_encode_pointer(_Result));
	return (_Result);
	}
...
unsigned long __stdcall _Not_supported_GetFinalPathNameByHandleW(HANDLE, wchar_t *, unsigned long, unsigned long)
 ...
 #define __vcrt_GetFinalPathNameByHandleW (_Runtime_dynamic_link( \
	 &GetFinalPathNameByHandleW_p, L"kernel32.dll", "GetFinalPathNameByHandleW", \
...
BOOL __stdcall _Not_supported_GetFileInformationByHandleEx(HANDLE, FILE_INFO_BY_HANDLE_CLASS, void *, unsigned long)
	{
	SetLastError(ERROR_NOT_SUPPORTED);
	return (0);
	}

 #define __vcrt_GetFileInformationByHandleEx (_Runtime_dynamic_link( \
	&_GetFileInformationByHandleEx_p, L"kernel32.dll", "GetFileInformationByHandleEx", \
...
BOOL __stdcall _Not_supported_SetFileInformationByHandle(HANDLE, FILE_INFO_BY_HANDLE_CLASS, void *, unsigned long)
 ...
 #define __vcrt_SetFileInformationByHandle (_Runtime_dynamic_link( \
	&_SetFileInformationByHandle_p, L"kernel32.dll", "SetFileInformationByHandle", \
...
#if defined(_CRT_APP) || defined(_KERNELX) || _STL_WIN32_WINNT < _WIN32_WINNT_VISTA
BOOLEAN __stdcall _Not_supported_CreateSymbolicLinkW(const wchar_t *, const wchar_t *, DWORD)
	{
	SetLastError(ERROR_NOT_SUPPORTED);
	return (0);
	}
#endif /* defined(_CRT_APP) || defined(_KERNELX) || _STL_WIN32_WINNT < _WIN32_WINNT_VISTA */
...
_CreateSymbolicLinkW_t _CreateSymbolicLinkW_p{};
 ...
 #define __vcrt_CreateSymbolicLinkW (_Runtime_dynamic_link( \
	&_CreateSymbolicLinkW_p, L"kernel32.dll", "CreateSymbolicLinkW", \
	&_Not_supported_CreateSymbolicLinkW))
#else /* Vista and non (App or KernelX) */
...
[[nodiscard]] __std_fs_copy_file_result __stdcall __vcp_Copyfile(const wchar_t * const _Source,
	const wchar_t * const _Target, const bool _Fail_if_exists) noexcept
	{
#if defined(_CRT_APP) || defined(_KERNELX)
...
[[nodiscard]] __std_win_error __stdcall _Create_symlink(const wchar_t * const _Symlink_file_name,
	const wchar_t * const _Target_file_name, const DWORD _Flags) noexcept
	{
	if (__vcrt_CreateSymbolicLinkW(_Symlink_file_name, _Target_file_name,
		_Flags | 0x2 /* SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE */) != 0)
		{
		return (__std_win_error::_Success);
		}
...
[[nodiscard]] __std_win_error __stdcall _Get_last_write_time_by_handle(const HANDLE _Handle,
	long long * const _Last_write_filetime)
...
#if _STL_WIN32_WINNT < _WIN32_WINNT_VISTA
	// note: "maybe XP" test also excludes _CRT_APP and _KERNELX
...
[[nodiscard]] __std_ulong_and_error __stdcall __std_fs_get_full_path_name(
	const wchar_t * _Source, unsigned long _Target_size, wchar_t * _Target) noexcept
...
[[nodiscard]] __std_win_error __stdcall __std_fs_open_handle(__std_fs_file_handle * const _Handle,
	const wchar_t * const _File_name, const __std_access_rights _Desired_access, const __std_fs_file_flags _Flags) noexcept
	{	// calls CreateFile2 or CreateFileW
	const HANDLE _Result = __vcp_CreateFile(_File_name, static_cast<unsigned long>(_Desired_access),
		FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
		nullptr, OPEN_EXISTING, static_cast<unsigned long>(_Flags), 0);
...
void __stdcall __std_fs_close_handle(const __std_fs_file_handle _Handle) noexcept
	{	// calls CloseHandle
...
[[nodiscard]] __std_win_error __stdcall __std_fs_get_file_attributes_by_handle(const __std_fs_file_handle _Handle,
	unsigned long * const _File_attributes) _NOEXCEPT
...
[[nodiscard]] __std_ulong_and_error __stdcall __std_fs_get_final_path_name_by_handle(
	const __std_fs_file_handle _Handle, wchar_t * const _Target, const unsigned long _Target_size,
	const __std_fs_volume_name_kind _Flags) noexcept
	...
	(void)_Target_size;
	(void)_Flags;
	return {0, __std_win_error::_Not_supported};
#else /* ^^^ _KERNELX ^^^ // vvv !_KERNELX vvv */
	const auto _Result = __vcrt_GetFinalPathNameByHandleW(reinterpret_cast<HANDLE>(_Handle), _Target,
		_Target_size, static_cast<unsigned long>(_Flags));
	return {_Result, _Result == 0 ? __std_win_error{GetLastError()} : __std_win_error::_Success};
#endif /* _KERNELX */
	}
...
[[nodiscard]] __std_win_error __stdcall __std_fs_directory_iterator_open(
	const wchar_t * const _Path_spec,
	__std_fs_dir_handle * const _Handle,
	__std_fs_find_data * const _Results) noexcept
	{
	__std_fs_directory_iterator_close(*_Handle);
	*_Handle = __std_fs_dir_handle{reinterpret_cast<intptr_t>(FindFirstFileExW(
		_Path_spec, FindExInfoBasic, _Results, FindExSearchNameMatch, nullptr, 0))};
	...
	// If the above call failed, we might be on pre Win7 / Server 2008R2, which doesn't support
	...
	*_Handle = __std_fs_dir_handle{reinterpret_cast<intptr_t>(FindFirstFileExW(
		_Path_spec, FindExInfoStandard, _Results, FindExSearchNameMatch, nullptr, 0))};
	if (*_Handle != __std_fs_dir_handle::_Invalid)
		{
		return (__std_win_error::_Success);
		}
...
[[nodiscard]] __std_fs_convert_result __stdcall __std_fs_convert_narrow_to_wide(const __std_code_page _Code_page,
	...
	{
	const int _Len = MultiByteToWideChar(static_cast<unsigned int>(_Code_page), MB_ERR_INVALID_CHARS,
		_Input_str, _Input_len, _Output_str, _Output_len);
...
[[nodiscard]] __std_fs_convert_result __stdcall __std_fs_convert_wide_to_narrow(const __std_code_page _Code_page,
	...
	if (_Code_page == __std_code_page{CP_UTF8} || _Code_page == __std_code_page{54936})
		{	// For UTF-8 or GB18030, attempt to use WC_ERR_INVALID_CHARS.
			// (These codepages can't use WC_NO_BEST_FIT_CHARS below,
			// and other codepages can't use WC_ERR_INVALID_CHARS.)
			// WC_ERR_INVALID_CHARS is Vista+. XP will activate the ERROR_INVALID_FLAGS fallback below.
		_Result._Len = WideCharToMultiByte(static_cast<unsigned int>(_Code_page), WC_ERR_INVALID_CHARS,
			_Input_str, _Input_len, _Output_str, _Output_len, nullptr, nullptr);
		}
	else
		{	// For other codepages, attempt to use WC_NO_BEST_FIT_CHARS.
			// Codepages that don't support this will activate the ERROR_INVALID_FLAGS fallback below.
		BOOL _Used_default_char = FALSE;

		_Result._Len = WideCharToMultiByte(static_cast<unsigned int>(_Code_page), WC_NO_BEST_FIT_CHARS,
			_Input_str, _Input_len, _Output_str, _Output_len, nullptr, &_Used_default_char);

		if (_Used_default_char)
			{	// Report round-tripping failure with ERROR_NO_UNICODE_TRANSLATION,
	...
	if (_Result._Err == __std_win_error{ERROR_INVALID_FLAGS})
		{	// Fall back to a non-strict conversion.
		_Result._Len = WideCharToMultiByte(static_cast<unsigned int>(_Code_page), 0,
			_Input_str, _Input_len, _Output_str, _Output_len, nullptr, nullptr);
...
[[nodiscard]] __std_fs_copy_file_result __stdcall __std_fs_copy_file(const wchar_t * const _Source,
	const wchar_t * const _Target, __std_fs_copy_options _Options) noexcept
	{	// copy _Source to _Target
	...
	if (_Options != __std_fs_copy_options::_Overwrite_existing)
		...
		// in exclusive mode, so a subsequent open will fail with ERROR_SHARING_VIOLATION.
			{
			const _STD _Fs_file _Source_handle(__vcp_CreateFile(_Source, FILE_READ_ATTRIBUTES | FILE_READ_DATA,
			...
			const _STD _Fs_file _Target_handle(__vcp_CreateFile(_Target, FILE_READ_ATTRIBUTES | FILE_WRITE_DATA,
				0, nullptr, OPEN_EXISTING, 0, 0));
			...
			if (_Source_last_write_time <= _Target_last_write_time)
				{	// _Target is newer, so don't update_existing
				return {false, __std_win_error::_Success};
				}
...
__std_win_error __stdcall __std_fs_get_file_id(__std_fs_file_id * const _Id, const wchar_t * const _Path) noexcept
	{
	__std_win_error _Last_error;
	const _STD _Fs_file _Handle(_Path,
		__std_access_rights::_File_read_attributes, __std_fs_file_flags::_Backup_semantics, &_Last_error);
	if (_Last_error != __std_win_error::_Success)
		{
		return (_Last_error);
		}
	...
	if (__vcrt_GetFileInformationByHandleEx(_Handle._Get(), FileIdInfo,
		reinterpret_cast<FILE_ID_INFO *>(_Id), sizeof(*_Id)) != 0)
...
[[nodiscard]] __std_win_error __stdcall __std_fs_create_hard_link(const wchar_t * const _File_name,
	const wchar_t * const _Existing_file_name) noexcept
	{
#if defined(_CRT_APP) || defined(_KERNELX)
	(void)_File_name;
...
[[nodiscard]] __std_win_error __stdcall __std_fs_read_reparse_data_buffer(
	...
	if (0 == DeviceIoControl(reinterpret_cast<HANDLE>(_Handle), FSCTL_GET_REPARSE_POINT, nullptr, 0,
			_Buffer, _Buffer_size, &_Bytes_returned, nullptr))
		{
		return (__std_win_error{GetLastError()});
		}
...
[[nodiscard]] __std_win_error __stdcall __std_fs_read_name_from_reparse_data_buffer(
	__std_fs_reparse_data_buffer * const _Buffer, wchar_t ** const _Offset,
...
[[nodiscard]] __std_win_error __stdcall __std_fs_set_last_write_time(const long long _Last_write_filetime,
	const wchar_t * const _Path) noexcept
	{
	__std_win_error _Last_error;
	const _STD _Fs_file _Handle(_Path, __std_access_rights::_File_write_attributes, __std_fs_file_flags::_None,
		&_Last_error);
	if (_Last_error != __std_win_error::_Success)
		{
		return (_Last_error);
...
[[nodiscard]] __std_win_error __stdcall __std_fs_change_permissions(const wchar_t * const _Path,
	const bool _Follow_symlinks, const bool _Readonly) noexcept
	{
	...
	if ((_Old_attributes & FILE_ATTRIBUTE_REPARSE_POINT) && _Follow_symlinks)
		...
		if ((_Basic_info.FileAttributes & FILE_ATTRIBUTE_READONLY) == _Readonly_test)
			{	// nothing to do
			return (__std_win_error::_Success);
			}
	...
	if ((_Old_attributes & FILE_ATTRIBUTE_READONLY) == _Readonly_test)
		{	// nothing to do
		return (__std_win_error::_Success);
		}
...
[[nodiscard]] __std_win_error __stdcall __std_fs_rename(const wchar_t * const _Source,
	const wchar_t * const _Target) noexcept
	{
...
[[nodiscard]] __std_win_error __stdcall __std_fs_space(const wchar_t * const _Target,
	uintmax_t * const _Available, uintmax_t * const _Total_bytes, uintmax_t * const _Free_bytes) noexcept
	{	// get capacity information for the volume on which the file _Target resides
	static_assert(sizeof(uintmax_t) == sizeof(ULARGE_INTEGER));
	static_assert(alignof(uintmax_t) == alignof(ULARGE_INTEGER));
	if (GetDiskFreeSpaceExW(_Target, reinterpret_cast<PULARGE_INTEGER>(_Available),
		...
		{	// optimize for valid input by not changing any parameters
		return (__std_win_error::_Success);
		}

	auto _Last_error = GetLastError();
	if (_Last_error == ERROR_DIRECTORY)
		{	// maybe failed because we had a file, try to strip that
		const auto _Target_end = _Target + _CSTD wcslen(_Target);
		auto _Dir_end = _Target_end;
		while (_Target != _Dir_end && ! _STD _Is_slash(_Dir_end[-1]))
			{	// back up to trailing slash
			--_Dir_end;
			}

		if (_Target != _Dir_end && _Dir_end != _Target_end)
			{	// there was a file name to remove, try that
			const auto _Keep_size = _Dir_end - _Target;
			auto _Dir_name = _malloc_crt_t(wchar_t, _Keep_size + 1);
			if (_Dir_name.is_valid())
				{
				_CSTD memcpy(_Dir_name.get(), _Target, _Keep_size * sizeof(wchar_t));
				_Dir_name.get()[_Keep_size] = L'\0';
				if (GetDiskFreeSpaceExW(_Dir_name.get(), reinterpret_cast<PULARGE_INTEGER>(_Available),
					reinterpret_cast<PULARGE_INTEGER>(_Total_bytes), reinterpret_cast<PULARGE_INTEGER>(_Free_bytes)))
					{
					return (__std_win_error::_Success);
					}
				...
				{
				_Last_error = ERROR_NOT_ENOUGH_MEMORY;
				}
			}
		}
...
[[nodiscard]] __std_ulong_and_error __stdcall __std_fs_get_temp_path(wchar_t * const _Target) noexcept
	{	// calls GetTempPathW
		// If getting the path failed, returns 0 size; otherwise, returns the size of the expected directory.
		// If the path could be resolved to an existing directory, returns __std_win_error::_Success; otherwise,
		// returns __std_win_error::_Max.
	...
	if (_Attributes & FILE_ATTRIBUTE_REPARSE_POINT)
		...
		const auto _Last_error = __std_fs_open_handle(&_Handle, _Target,
			__std_access_rights::_File_read_attributes, __std_fs_file_flags::_Backup_semantics);
...
[[nodiscard]] __std_win_error __stdcall __std_fs_get_stats(const wchar_t * const _Path, __std_fs_stats * const _Stats,
	...
	if (_Bitmask_includes(_Flags, __std_fs_stats_flags::_Attributes)
		...
		&& (!_Bitmask_includes(_Symlink_attribute_hint, __std_fs_file_attr::_Reparse_point) || !_Follow_symlinks))
		{	// if the hint can't be a symlink, we already have the attributes
		_Flags &= ~__std_fs_stats_flags::_Attributes;
		_Stats->_Attributes = _Symlink_attribute_hint;
		}

	if (_Flags == __std_fs_stats_flags::_None)
		{	// no more data to get, report success
		return (__std_win_error::_Success);
		}

	constexpr auto _Get_file_attributes_data =
		__std_fs_stats_flags::_Attributes
		| __std_fs_stats_flags::_File_size
		| __std_fs_stats_flags::_Last_write_time;
	if (_Bitmask_includes(_Flags, _Get_file_attributes_data))
		{	// caller wants something GetFileAttributesExW might provide
		if (_Symlink_attribute_hint == __std_fs_file_attr::_Invalid
			...
			|| !_Follow_symlinks)
			{	// we might not be a symlink or not following symlinks, so GetFileAttributesExW would return
				// the right answer
			...
			if (!_Follow_symlinks
				|| !_Bitmask_includes(_Data._Attributes, __std_fs_file_attr::_Reparse_point))
	...
	if (_Get_info_fn._Supported())
		{
		constexpr auto _Basic_info_data =
			__std_fs_stats_flags::_Attributes | __std_fs_stats_flags::_Last_write_time;
		constexpr auto _Attribute_tag_info_data =
			__std_fs_stats_flags::_Attributes | __std_fs_stats_flags::_Reparse_tag;
		constexpr auto _Standard_info_data =
	...
	_Stats->_File_size = (static_cast<unsigned long long>(_Legacy_info.nFileIndexHigh) << 32)
		+ _Legacy_info.nFileIndexLow;
	_Stats->_Link_count = _Legacy_info.nNumberOfLinks;
	_Flags &= ~(__std_fs_stats_flags::_Attributes
		| __std_fs_stats_flags::_Last_write_time
		| __std_fs_stats_flags::_File_size
		| __std_fs_stats_flags::_Link_count);
...
#else /* ^^^ defined(_KERNELX) || defined(_CRT_APP) ^^^ // vvv !defined(_KERNELX) && !defined(_CRT_APP) vvv */
...
[[nodiscard]] __std_ulong_and_error __stdcall __std_fs_get_current_path(const unsigned long _Target_size,
	wchar_t * const _Target) noexcept
	{
	// If getting the path failed, GetCurrentDirectoryW returns 0; otherwise, returns the size of the expected directory.
