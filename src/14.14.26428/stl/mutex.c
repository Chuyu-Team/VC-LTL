...
__stl_sync_api_modes_enum __stl_sync_api_impl_mode
	= __stl_sync_api_modes_enum::normal;

extern "C" _CRTIMP2 void __cdecl __set_stl_sync_api_mode(
	__stl_sync_api_modes_enum mode)
	{	/* set behavior to normal/win7/vista/concrt */
...
struct _Mtx_internal_imp_t
	...
	std::aligned_storage_t<
		Concurrency::details::stl_critical_section_max_size,
		Concurrency::details::stl_critical_section_max_alignment> cs;
	long thread_id;
	int count;
	Concurrency::details::stl_critical_section_interface *_get_cs()
		{	/* get pointer to implementation */
		return (reinterpret_cast<Concurrency::details::
			stl_critical_section_interface *>(&cs));
		}
	};

static_assert(sizeof(_Mtx_internal_imp_t) <= _Mtx_internal_imp_size,
	"incorrect _Mtx_internal_imp_size");
static_assert(std::alignment_of<_Mtx_internal_imp_t>::value
	<= _Mtx_internal_imp_alignment, "incorrect _Mtx_internal_imp_alignment");

void _Mtx_init_in_situ(_Mtx_t mtx, int type)
	{	/* initialize mutex in situ */
...
void _Mtx_destroy_in_situ(_Mtx_t mtx)
	{	/* destroy mutex in situ */
