...
namespace details
{
    ...
    public:
        ...
        public:
            ...
            bool DeferToAffineSearchers() const
            {
                ...
                return (m_pSegment->m_pScheduler->HasSearchers(m_pSegment->m_affinitySet) &&
                        !m_pSegment->m_affinitySet.IsSet(pContext->m_pVirtualProcessor->GetMaskId()));
            }
        ...
        Mailbox(SchedulerBase *pScheduler, const QuickBitSet&, bool fDeferAlloc = false, unsigned int segmentSize = s_segmentSize) :
            ...
            m_head(0), m_tail(0),
            m_pTailSegment(NULL), m_pHeadSegment(NULL)
        {
            ...
            if (!fDeferAlloc)
                m_pHeadSegment = m_pTailSegment = _concrt_new Segment(m_pScheduler, m_affinitySet, segmentSize, 0);
        }
