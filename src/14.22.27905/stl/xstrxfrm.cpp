/***
 *xstrxfrm.cpp - Transform a string using locale information
 *
...
_CRTIMP2_PURE size_t __CLRCALL_PURE_OR_CDECL _Strxfrm(
    ...
    if ((locale_name == NULL) && (codepage == CP_ACP)) {
        if (_n2 <= _n1) {
        ...
        if (0
            != (dstlen = __crtLCMapStringA(locale_name, LCMAP_SORTKEY, _string2, (int) _n2, NULL, 0, codepage, TRUE))) {
            retval = dstlen;
