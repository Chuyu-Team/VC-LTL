...
)
{
    ...
    if (pDC->ScopeIndex != 0)
    {
        ...
        DASSERT(CatchStateInParent == INVALID_CATCH_SPECIFIC_STATE);
            ...
            if (dispAction != 0) {
                ...
                __try {
                    ...
                    {   // TODO: this is currently hardcoded for non-x86, as x86 seems to encode
                        ...
                        RENAME_EH_EXTERN(_CallSettingFrame)((void*)__RVAtoRealOffset(pDC, dispAction),
...
#else
            DASSERT( false ); // FH4 is only implemented for amd64
