...
namespace details
{
        ...
        m_lastServiceScan(0)
    {
        ...
        m_dwExternalStatisticsIndex = platform::__TlsAlloc();
    ...
    void SchedulerBase::Initialize()
    {
        ...
        m_hSchedulerShutdownSync = __crtCreateSemaphoreExW(NULL, 0, 0x7FFFFFFF, NULL, 0, SEMAPHORE_ALL_ACCESS);
    ...
    void SchedulerBase::ChangeThrottlingTimer(ULONG dueTime)
    {
        if (Concurrency::GetOSVersion() >= Concurrency::IResourceManager::Vista)
        {
            // The ChangeTimerQueueTimer is a pre-Vista API that is not available for _KERNELX
            FILETIME time = {0};
            ...
            __crtSetThreadpoolTimer(static_cast<PTP_TIMER>(m_hThrottlingTimer), &time, PSEUDOINFINITE, 0);
        }
        else
        {
