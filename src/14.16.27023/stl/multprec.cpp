// multprec: implements multiprecision math for random number generators
#include <limits>
#include <random>
...
_NODISCARD unsigned long long __CLRCALL_PURE_OR_CDECL _MP_Get(_MP_arr u) noexcept
	{	// convert multi-word value to scalar value
	return ((u[1] << shift) + u[0]);
	}

static void add(unsigned long long *u, int ulen, unsigned long long *v, int vlen) noexcept
...
void __CLRCALL_PURE_OR_CDECL _MP_Add(_MP_arr u, unsigned long long v0) noexcept
...
static void mul(unsigned long long* u, int ulen, unsigned long long v0) noexcept
...
void __CLRCALL_PURE_OR_CDECL _MP_Mul(_MP_arr w, unsigned long long u0, unsigned long long v0) noexcept
...
static void div(_MP_arr u, unsigned long long v0) noexcept
	{	// divide multi-word value by scalar value (fits in lower half of unsigned long long)
	unsigned long long k = 0;
	int ulen = _MP_len;
	while (0 <= --ulen)
...
_NODISCARD static int limit(const unsigned long long* u, int ulen) noexcept
...
void __CLRCALL_PURE_OR_CDECL _MP_Rem(_MP_arr u, unsigned long long v0) noexcept
