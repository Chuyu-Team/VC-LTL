...
extern "C" _CRTIMP2 void __cdecl __set_stl_sync_api_mode(
    __stl_sync_api_modes_enum mode) { // set behavior to normal/win7/vista/concrt
...
int _Mtx_init(_Mtx_t* mtx, int type) { // initialize mutex
    ...
    *mtx = 0;
    ...
    if ((mutex = (_Mtx_t) _calloc_crt(1, sizeof(struct _Mtx_internal_imp_t))) == 0) {
...
static int mtx_do_lock(_Mtx_t mtx, const xtime* target) { // lock mutex
    if ((mtx->type & ~_Mtx_recursive) == _Mtx_plain) { // set the lock
        ...
        } else if (target->sec < 0
                   || target->sec == 0 && target->nsec <= 0) { // target time <= 0 --> plain trylock or timed wait for
