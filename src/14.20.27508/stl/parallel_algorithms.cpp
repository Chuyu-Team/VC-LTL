...
namespace {
    ...
    inline unsigned char _Atomic_load_uchar(const volatile unsigned char*
            _Ptr) noexcept { // atomic load of unsigned char, copied from <atomic> except ARM and ARM64 bits
...
#error Unsupported architecture
    ...
    inline unsigned int _Atomic_load_uint(const volatile unsigned int*
            _Ptr) noexcept { // atomic load of unsigned int, copied from <atomic> except ARM and ARM64 bits
        unsigned int _Value;
#if defined(_M_IX86) || defined(_M_X64)
        _Value = *_Ptr;
        _ReadWriteBarrier();
...
#error Unsupported architecture
    ...
    inline void _Atomic_store_uint(
        volatile unsigned int* _Tgt, unsigned int _Value) { // atomic store of unsigned int, copied from <atomic>
#if defined(_M_IX86) || defined(_M_X64)
...
#error Check hardware assumption: Assumes that write races of identical values to pointer-sized variables are benign
        ...
        if (!_Parallel_info._Pfn_CreateThreadpoolWork || !_Parallel_info._Pfn_SubmitThreadpoolWork
            || !_Parallel_info._Pfn_CloseThreadpoolWork
            || !_Parallel_info._Pfn_WaitForThreadpoolWorkCallbacks) { // don't parallelize without the Vista threadpool
            return false;
        }
#endif // _STL_WIN32_WINNT < _WIN32_WINNT_VISTA
        ...
        if (_KernelBase) {
            ...
            if ((_Parallel_info._Pfn_WaitOnAddress == nullptr)
                != (_Parallel_info._Pfn_WakeByAddressAll == nullptr)) { // if we don't have both we can use neither
        ...
        if (!_Parallel_info._Pfn_AcquireSRWLockExclusive || !_Parallel_info._Pfn_ReleaseSRWLockExclusive
            || !_Parallel_info._Pfn_SleepConditionVariableSRW
            || !_Parallel_info
                    ._Pfn_WakeAllConditionVariable) { // no fallback for WaitOnAddress; shouldn't be possible as these
                                                      // APIs were added at the same time as the Vista threadpool API
            return false;
        }
#endif // _STL_WIN32_WINNT < _WIN32_WINNT_VISTA
...
#if _STL_WIN32_WINNT >= _WIN32_WINNT_WIN8
    if (WaitOnAddress(const_cast<volatile unsigned char*>(_Address), &_Compare, 1, INFINITE)
        == FALSE) { // this API failing should only be possible with a timeout, and we asked for INFINITE
        ::terminate();
    }
#else // ^^^ Win8 ^^^ // vvv pre-Win8 vvv
