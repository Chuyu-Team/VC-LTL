...
_EXTERN_C [[nodiscard]] __std_ulong_and_error __stdcall __std_fs_get_full_path_name(
...
[[nodiscard]] __std_fs_convert_result __stdcall __std_fs_convert_wide_to_narrow(const __std_code_page _Code_page,
    ...
    if (_Code_page == __std_code_page{CP_UTF8} || _Code_page == __std_code_page{54936}) {
        ...
        // below, and other codepages can't use WC_ERR_INVALID_CHARS.) WC_ERR_INVALID_CHARS is Vista+. XP will activate
        // the ERROR_INVALID_FLAGS fallback below.
...
[[nodiscard]] __std_fs_remove_result __stdcall __std_fs_remove(
    const wchar_t* const _Target) noexcept { // remove _Target without caring whether _Target is a file or directory
    __std_win_error _Last_error;
...
#define _SetFileInformationByHandle SetFileInformationByHandle
    ...
    if (_SetFileInformationByHandle == _Not_supported_SetFileInformationByHandle) { // Windows XP
        if (RemoveDirectoryW(
                _Target)) { // try RemoveDirectoryW first because it gives a specific error code for "the input was a
                            // file"; DeleteFileW on a directory input returns ERROR_ACCESS_DENIED
            return {true, __std_win_error::_Success};
        }
...
// TRANSITION, abi
// The following function exists only for link compatibility with older objs
