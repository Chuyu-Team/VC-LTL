...
_CRTIMP2_PURE int _Thrd_start(_Thrd_t* thr, _Thrd_callback_t func, void* b) { // start a thread
    return (thr->_Hnd = (HANDLE) _beginthreadex(0, 0, func, b, 0, &thr->_Id)) == 0 ? _Thrd_error : _Thrd_success;
}
...
_CRTIMP2_PURE _Thrd_t _Thrd_current(void) { // return _Thrd_t identifying current thread
    _Thrd_t result;
    result._Hnd = NULL;
...
_Thrd_id_t _Thrd_id(void) { // return unique id for current thread
    return GetCurrentThreadId();
}

unsigned int _Thrd_hardware_concurrency(void) { // return number of processors
    SYSTEM_INFO info;
