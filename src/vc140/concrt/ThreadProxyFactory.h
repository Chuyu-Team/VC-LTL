...
#pragma warning (disable : 4702) // unreachable code
    ...
    protected:
        ...
        virtual void Prepare(threadProxy *pProxy, int contextPriority)
        {
            ...
            {
                // All the threads in MSDK (they are really thread pool work item priority) have
                // the same priority. We do not allow the users to create schedulers with different
                // thread priority for modern apps or _KERNELX apps.
                ASSERT_NOT_APP_OR_KERNELX();
