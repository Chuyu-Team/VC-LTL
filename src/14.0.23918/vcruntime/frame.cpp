...
)
{
    ...
    NextTryBlock: ;
        ...
        // we are not handling a re-throw. Note: IsRethrow is true when we immediately process
        // the re-throw, this function is recursively entered. We will still return back to the
        // outer search with IsRethrow set to false after failing to find a handler here, and at
        // that point it is appropriate to terminate if we didn't find a handler (VSO 156464).
        // e.g.:
        // try { 
        //   try { throw 1; } (1)
        //   catch(int) { throw; } (2)
        ...
        // we find and execute catch handler (2). This re-throws, which re-enters this function
        // with IsRethrow = true but which cannot find a handler. We cannot terminate here, we're
        // in a recursive context. Just let it go, and we'll return back to the original search
        // from (1) and find (3). If we fail to find a handler, e.g. (3) didn't match, we would
        // then terminate at this point as IsRethrow is false. CatchDepth appears appropriate
        // to use to detect this case, but is not set correctly on AMD64.
        if( !gotMatch && FUNC_MAGICNUM(*pFuncInfo) >= EH_MAGIC_HAS_ES && 
            (FUNC_PESTYPES(pFuncInfo) != nullptr || (((FUNC_FLAGS(*pFuncInfo) & FI_EHNOEXCEPT_FLAG) != 0) && !IsRethrow) ))
        {
            // Are we noexcept?
