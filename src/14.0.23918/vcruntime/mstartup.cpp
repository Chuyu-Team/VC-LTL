...
private:
    ...
    inline void InitializePerAppDomain()
    {
        ...
        // Force-load the AppCRT, DesktopCRT, and the msvcp DLL before we call
        // into any user code.  We need to ensure that the AppCRT and DesktopCRT
        // are both loaded and initialized before we begin using either of them.
        // If we do not force-load them here, they are loaded on demand the first
        // time we use a function from one of them.  If the first function call
        // to one of them is into the AppCRT, only the AppCRT will load then, so
        // the post-initialization of the AppCRT done by the DesktopCRT will not
        // take place.
        //
        // We must also load msvcp immediately.  Otherwise, it will load the first
        // time user code calls into the DLL, and this delayed loading may cause
        // unexpected reentrancy into code defined in the STL headers as global
        // initializers are processed for the msvcp DLL.
        //
        // To force-load the DLLs, we simply call one function from the DesktopCRT
        // and one function from msvcp.
        _getdrive();
        xtime_get(nullptr, 0);
