...
// Private defintion of AgileReferenceOptions workaround
// incorrect settings for NTDDI_VERSION flag in msbuild
...
#if defined _VCCORLIB_APP && !defined _KERNELX
_Check_return_ WINOLEAPI RoGetAgileReference(
...
static HRESULT GetAgileReference(IUnknown* pUnk, _COM_Outptr_ IAgileReference **ppAgileReference)
{
    return RoGetAgileReference(AGILEREFERENCE_DEFAULT, __uuidof(IUnknown), pUnk, ppAgileReference);
}
#else // ^^^ _VCCORLIB_APP && !_KERNELX ^^^ // vvv !_VCCORLIB_APP || _KERNELX vvv //
typedef HRESULT (__stdcall *PFN_RO_GET_AGILE_REFERENCE)(
        _In_ enum AgileReferenceOptions options,
        _In_ REFIID riid,
        _In_ IUnknown * pUnk,
        _COM_Outptr_ IAgileReference ** ppAgileReference
...
volatile void *RoGetAgileReferenceFunc = (volatile void*)::EncodePointer(nullptr);

static HRESULT GetAgileReference(IUnknown* pUnk, _COM_Outptr_ IAgileReference **ppAgileReference)
{
    HRESULT hr = E_FAIL;

    void* funcPtr = ::DecodePointer((void*)RoGetAgileReferenceFunc);
    if (funcPtr  != nullptr)
    {
    PFN_RO_GET_AGILE_REFERENCE pfnFunc = reinterpret_cast<PFN_RO_GET_AGILE_REFERENCE>(funcPtr);
        hr = pfnFunc(AGILEREFERENCE_DEFAULT, __uuidof(IUnknown), pUnk, ppAgileReference);
    }
    else
    {
        HMODULE hMod = GetModuleHandle(L"combase.dll");
        if (hMod)
        {
            PFN_RO_GET_AGILE_REFERENCE pfnFunc = reinterpret_cast<PFN_RO_GET_AGILE_REFERENCE>(GetProcAddress(hMod, "RoGetAgileReference"));
            if (pfnFunc != nullptr)
            {
                RoGetAgileReferenceFunc = ::EncodePointer(pfnFunc);
                hr = pfnFunc(AGILEREFERENCE_DEFAULT, __uuidof(IUnknown), pUnk, ppAgileReference);
            }
        }
    }
...
class PerApartmentFactoryCache : public RuntimeClass<RuntimeClassFlags<ClassicCom>, IApartmentShutdown, FtmBase>
{
    UINT64 _apartmentId;
protected:
    typedef unique_ptr<const wchar_t, conditional_deleter> ActivatableID;
    typedef pair<bool, ComPtr<IUnknown> > CacheEntry; // bool is true in case that the object is IAgileReference
private:
...
protected:
    virtual HRESULT AddFactoryInternal(std::pair<ActivatableID, CacheEntry> &&entry)
    {
    ...
    virtual HRESULT RemoveFactoryInternal(const wchar_t* acid)
    {
        // Don't delete buffer
    ...
    virtual HRESULT GetFactoryInternal(const wchar_t* acid, Platform::Guid& iid, void** pFactory, bool& addToCache)
    {
        ...
        *pFactory = nullptr;
        ...
        if (it->second.first)
        {
            IAgileReference* agileRef = reinterpret_cast<IAgileReference*>(it->second.second.Get());
            return agileRef->Resolve(iid, pFactory);
        }
        else
        {
            return it->second.second.Get()->QueryInterface(iid, pFactory);
        }       
    }
public:
    ...
    HRESULT AddFactory(const wchar_t* acid, IUnknown *factory)
    {
        try
        {
            ...
            // Non agile object
            if (factory->QueryInterface(__uuidof(::IAgileObject), &agile) != S_OK)
            {
                ComPtr<IAgileReference> ref;
                HRESULT hr = GetAgileReference(factory, &ref);
                if (FAILED(hr))
                {
                    return hr;
                }
                
                temp = make_pair(true, ref.Get());
            }
            else
            {
                temp = make_pair(false, factory);
            }
           
            auto entry = make_pair(std::move(activatableID), std::move(temp));
            return AddFactoryInternal(std::move(entry));
        }
    ...
    HRESULT GetFactory(const wchar_t* acid, Platform::Guid& iid, void** pFactory, bool& addToCache)
    {
        *pFactory = nullptr;
        try
        {
            // Remove factory in case of RPC error
            HRESULT hr = GetFactoryInternal(acid, iid, pFactory, addToCache);
            if (IsRPCError(hr)) 
            {
                RemoveFactoryInternal(acid);
            }
...
protected:
    ...
    virtual HRESULT RemoveFactoryInternal(const wchar_t* acid)
    {
        ...
        // Try to lock exclusive in case of removal (RPC errors)
        // In worst case scenario user will have to go over cache logic
        // once again and try to remove factory again if cannot acquire the lock
        auto lock = _SRWLock.TryLockExclusive();
        if (!lock.IsLocked())
        {
            return E_FAIL;
        }

        return PerApartmentFactoryCache::RemoveFactoryInternal(acid);
    }
