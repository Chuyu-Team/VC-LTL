...
template<class _Alloc,
	...
	return (_STD tuple_cat(tuple<allocator_arg_t>(allocator_arg),
		tuple<_Alloc&>(_Alty), _STD move(_Val)));
	}
...
public:
	...
	template<class _Ty1,
		...
		{	// construct pair from tuples
		_Scoped_construct_pair(inner_allocator(), _Ptr, _STD move(_Val1), _STD move(_Val2));
		}
