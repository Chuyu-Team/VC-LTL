...
public:
    ...
    typename _Alnode::value_type::value_type& _Allocate_if_necessary(
        typename _Alnode::value_type::value_type& _Val) const noexcept {
        return _Val;
    }

    _Nodeptr _Transfer_before(const _Nodeptr _Rightnode) noexcept {
        const _Nodeptr _Result   = _Release();
        const _Nodeptr _Leftnode = _Rightnode->_Prev;
        _Result->_Next           = _Rightnode;
        _Result->_Prev           = _Leftnode;
        _Rightnode->_Prev        = _Result;
        _Leftnode->_Next         = _Result;
        return _Result;
    }
