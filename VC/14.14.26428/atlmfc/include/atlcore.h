...
namespace ATLImplementationDetails
{
	...
	template<class T, T Val> struct integral_constant { static constexpr T value = Val; };
	...
	template<class T> add_rvalue_reference_t<T> declval() throw();

	template<class T>
	constexpr T&& forward(remove_reference_t<T>& arg) throw()
	{
	...
	template <class Ty>
	using is_nothrow_move_constructible = is_nothrow_constructible<Ty, Ty>;
	...
	template <class Ty>
	using is_nothrow_move_assignable = is_nothrow_assignable<add_lvalue_reference_t<Ty>, Ty>;
